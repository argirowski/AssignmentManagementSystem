...
namespace Persistence.Repositories
{
    public class StatusRepository : IStatusRepository
    {
        ...
        public async Task<Status?> GetByIdAsync(Guid id)
        {
            ...
            return await _connection.QuerySingleOrDefaultAsync<Status>(query, new { Id = id });
        }

        public async Task<IEnumerable<Status>> GetAllAsync()
        {
            const string query = "SELECT * FROM Statuses";
            return await _connection.QueryAsync<Status>(query);
        }

        public async Task AddAsync(Status status)
        {
            const string query = "INSERT INTO Statuses (Id, Description) VALUES (@Id, @Description)";
            await _connection.ExecuteAsync(query, status);
        }
        ...
        public async Task<bool> IsStatusLinkedToAssignmentsAsync(Guid statusId)
        {
            ...
            var count = await _connection.ExecuteScalarAsync<int>(query, new { StatusId = statusId });
            return count > 0;
        }
        ...
        public async Task<Status?> GetByDescriptionAsync(string description)
        {
            ...
            return await _connection.QueryFirstOrDefaultAsync<Status>(query, new { Description = description });
        }
    }
}